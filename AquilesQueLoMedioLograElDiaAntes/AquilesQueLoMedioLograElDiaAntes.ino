...
boolean GoneR, GoneL, GoneVR, GoneVL, GotLost, GoingF, Intersection, Center, Angle;
int SensorCount;
...
void loop() {
  ...
  int NormalSpeed = MotorSpeed * 1.8;
  int LowSpeed = MotorSpeed * 1.6;
  int CurveSpeed = MotorSpeed * 1.4;
  ...
  while ( !StartEngine && analogRead(A0) < 900 ) {
   if  (analogRead(A0) > 900) {StartEngine = true;}
  }
  
  if  (analogRead(A0) > 900) {delay(2000);}*/
...
if (GotLost) {
  if (GoneR) {
    reverseMotors(1);
    runMotors(2);
    motorSpeed (NormalSpeed, CurveSpeed);
  } else if (GoneL) {
    runMotors(1);
    reverseMotors(2);
    motorSpeed (CurveSpeed, NormalSpeed);
  }
}
